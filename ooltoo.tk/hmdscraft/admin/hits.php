...
include '../common.php';
$total_hit=0;
$retval=sql_query("SELECT * FROM hits ORDER BY uri");
while($row=mysql_fetch_array($retval)){
	...
	$other=$row['other'];
	$total_hit +=$hit;
	echo "<tr><td>{$id}</td><td>{$uri}</td><td>{$hit}</td><td><span onclick=hitreset('{$uri}')>RESET</span></td> <td>---<span onclick=delete_uri('{$uri}')>DELETE</span></td></tr>";
	}echo $total_hit;
?>
